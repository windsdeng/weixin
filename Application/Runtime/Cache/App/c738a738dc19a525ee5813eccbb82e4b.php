...
<body class="sanckbg mode_webapp">
	...
	<div id="member-container" style="display: block;">
		...
		<div class="div_table" style='background-color:#e61945;height:20px;padding:10px;'>
			<table>
				<tr><td><?php echo ($count_desc); ?></td></tr>
			</table>
		</div>
			...
			<ul class="round_user">
				<?php if(is_array($result)): $i = 0; $__LIST__ = $result;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$result): $mod = ($i % 2 );++$i;?><li>
						<div style="">
							...
							<?php $wx_info = json_decode($result['wx_info'],true); $img = !empty($wx_info['headimgurl'])?$wx_info['headimgurl']:'../Public/Static/images/defult.jpg'; echo "<img src='".$img."' width='40px;' height='40px;'>"; ?>
							</span>
							<span class="header_right" style="margin-top:5px;">
								<div class="header_l_di"><span>昵称：<?php echo $wx_info['nickname']; ?></span></div>
								<div><span>合伙人：<?php if($result["member"] == 1): ?>是<?php else: ?>否<a style='color:red' href='./index.php?g=App&m=Index&a=index'>点击链接成为合伙人</a><?php endif; ?></span></div>
								<div><span>关注时间：<?php echo date('Y-m-d',$wx_info['subscribe_time']); ?></span></div>
								<div><span>会员ID：<?php echo ($result["id"]); ?>  </span></div>
							</span>
						</div>
